...
def normalize_akoya_vectorized(
    akoya_folder,
    akoya_masked,
    kfbio_folder,
    kfbio_masked,
    ...
    for akoya_path in akoya_paths:
        akoya_name = os.path.splitext(os.path.basename(akoya_path))[0]
        akoya_mask_path = os.path.join(akoya_masked, f"{akoya_name}_MASK.tif")

        akoya_img = load_tiff_with_tifffile(akoya_path, color=True, scale_factor=scale_factor)
        akoya_mask = load_mask_pil(akoya_mask_path, scale_factor=scale_factor)

        akoya_vector = akoya_img[akoya_mask]
        print(f"Loaded and vectorized Akoya image: {akoya_path}")

        chosen_refs = random.sample(kfbio_paths, num_refs)
        ref_vectors = []

        for ref_path in chosen_refs:
            ref_name = os.path.splitext(os.path.basename(ref_path))[0]
            ref_mask_path = os.path.join(kfbio_masked, f"{ref_name}_MASK.tif")
        ...
        akoya_normed_vector = normalizer.transform(akoya_vector.reshape(-1, 1, 3), return_mask=False)
        akoya_normed_vector_np = akoya_normed_vector.cpu().numpy().reshape(-1, 3).astype(np.uint8)

        akoya_normed_img = np.zeros_like(akoya_img, dtype=np.uint8)
        akoya_normed_img[akoya_mask] = akoya_normed_vector_np

        ref_list_str = "_".join([os.path.splitext(os.path.basename(r))[0] for r in chosen_refs])
        out_name = f"{akoya_name}__vectorized__{num_refs}refs__{ref_list_str}_normalized.tiff"
        out_path = os.path.join(output_folder, out_name)

        tifffile.imwrite(out_path, akoya_normed_img)
        print(f"Saved vectorized normalized tiff: {out_path}")

        png_out = out_path.replace(".tiff", ".png")
        cv2.imwrite(png_out, cv2.cvtColor(akoya_normed_img, cv2.COLOR_RGB2BGR))
